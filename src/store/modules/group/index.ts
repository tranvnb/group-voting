...
const group: Module<IGroupState[], IRootState> = {
  ...
  actions: {
    [GROUP.ACTION_ADD_VOTE_TO_GROUP]: (
      context: ActionContext<IGroupState[], IRootState>,
      payload: { groupId: number; isUpVote: boolean }
    ): void => {
      // side effect here
      context.commit(GROUP.MUTATION_ADD_VOTE, payload);
    },
    [GROUP.ACTION_TOGGLE_GROUP_STATE]: (
      context: ActionContext<IGroupState[], IRootState>,
      payload: { groupId: number; isUpVote: boolean }
    ): void => {
  ...
  mutations: {
    ...
    [GROUP.MUTATION_UPDATE_GROUP_VOTE]: (
      state: IGroupState[],
      payload: { groupId: number; isUpVote: boolean }
    ): void => {
